...
try {
    ...
    # Commande MySQL pour créer l'utilisateur admin
    $sqlCommands = @"
...
"@
    ...
    if ($dbPassword) {
        $result = $sqlCommands | mysql -h $dbHost -u $dbUser -p$dbPassword $dbName
    } else {
        $result = $sqlCommands | mysql -h $dbHost -u $dbUser $dbName
    }
