...
try {
    ...
    if ($loginResponse.StatusCode -eq 200) {
        ...
        if ($loginResult.token) {
            Write-Host "✅ Connexion Admin: OK (Token recu)" -ForegroundColor Green
        } else {
...
Write-Host "`n🎉 L'interface d'administration TESSA COIFFURE est prete !" -ForegroundColor Green
